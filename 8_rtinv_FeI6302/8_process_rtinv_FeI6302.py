...
nwav = 9
wave_steps = np.arange(-nwav+5, 5)*wave_step*1e3
config['science'][line+'/nwav'] = nwav
config.write()
...
#
tfile = open(invdir+os.sep+'hellride3.map', 'w')
...
noise = 0.001
#
tfile = open(invdir+os.sep+'hellride3.line', 'w')
...
tfile.write('\n')
tfile.write('telluric ini 8 \n')
tfile.write('telluric end 9 \n')
...
tfile = open(invdir+os.sep+'hellride3.filter', 'w')
...
tfile.close()
sp.run(f'mpirun -machinefile {invdir}/hostfile -n 16 {invdir}/vfisv_fpi.x -map={invdir}/hellride3.map -line={invdir}/hellride3.line -filter={invdir}/hellride3.filter', shell=True)

# %%
# sc = pf.open(pol_files[-1])[1].data
# show_img_series(sc[:,:,0,-nwav::], fps=1)
